...
def get_baseline(all=True):
	...
	print('Baseline Training Score: RMSE: %e' % math.sqrt(train_score))
	pred=test_x
	test_score = mean_squared_error(test_y, pred)
	#print('Baseline Test Score: RMSE: %s' % '{:,.0f}'.format(math.sqrt(test_score)))
	print('Baseline Test Score: RMSE: %e' % math.sqrt(test_score))

	bttscore='RMSE: %e/%e' % (math.sqrt(train_score), math.sqrt(test_score))
